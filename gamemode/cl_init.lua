...
include("cl_atmos.lua");
include("cl_cards.lua");
...
function GM:PlayerBindPress( pl, bind, down )
	if(bind == "+attack" and WEPSWITCH.show == true)then
		WEPSWITCH:SelectWep()
...
---------------------------------------------------------]]--
function GM:OnPlayerChat( plyr, strText, bTeamOnly, bPlayerIsDead )
	...
	if(plyr:Team() == 99 && LocalPlayer():Team() != 99 )then -- If both are not dead
		return false
	end
	...
	if(plyr:Team() == LocalPlayer():Team() && shouldShowTeam)then
		for k,v in pairs(CLASS_CONFIG.Classes)do
			local isLeader = v["leader"] or false
			if(v["name"] == player_manager.GetPlayerClass(plyr) && isLeader == true)then
				chat.AddText( team.GetColor(plyr:Team()), plyr, MAIN_CONFIG.chat_leaderColor, ": ", strText )
				return true
			end
		end
		local textColor = Color(255,255,255)
		chat.AddText( team.GetColor(plyr:Team()), plyr, textColor, ": ", strText )
		return true
	end
	
	if(bTeamOnly && shouldShowTeam == false)then
		return false
	end
	
	chat.AddText( Color(230,230,230), plyr, Color(255,255,255), ": ", strText )
...
function GM:Move()
	if(WEPSWITCH != nil)then
		WEPSWITCH.scroll = LocalPlayer():GetCurrentCommand():GetMouseWheel()
		WEPSWITCH:ScrollCheck()
	end
end
...
function GM:CreateMove( cmd )
	...
	if(cmd:GetButtons(IN_ATTACK) && WEPSWITCH.show)then
		return false
	end
