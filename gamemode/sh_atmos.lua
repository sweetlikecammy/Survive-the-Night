...
atmos_dnc_length_day = CreateConVar( "atmos_dnc_length_day", "3600", bit.bor( FCVAR_ARCHIVE, FCVAR_GAMEDLL, FCVAR_REPLICATED ), "The duration modifier of daytime in seconds." );
atmos_dnc_length_night = CreateConVar( "atmos_dnc_length_night", "3600", bit.bor( FCVAR_ARCHIVE, FCVAR_GAMEDLL, FCVAR_REPLICATED ), "The duration modifier of nighttime in seconds." );
...
atmos_weather_delay = CreateConVar( "atmos_weather_delay", "600", bit.bor( FCVAR_ARCHIVE, FCVAR_GAMEDLL, FCVAR_REPLICATED ), "How many seconds it takes for atmos to do a dice roll for weather, default is 600 (10 minutes), which means atmos will attempt to trigger a storm every 10 minutes using the chance value." );
atmos_weather_lightstyle = CreateConVar( "atmos_weather_lighting", "1", bit.bor( FCVAR_ARCHIVE, FCVAR_GAMEDLL, FCVAR_REPLICATED, FCVAR_NOTIFY ), "Allow weather to change the lighting (can be buggy?)." );

atmos_snowenabled = CreateConVar( "atmos_snowenabled", "1", bit.bor( FCVAR_ARCHIVE, FCVAR_GAMEDLL, FCVAR_REPLICATED, FCVAR_NOTIFY ), "Atmos Snow enabled." );
